<?php
	...
	class database {
		...
		function __construct() {
			...
			$this->db = 'pc_database'; // pc_database is our database name in phpmyadmin
			...
			if(is_array($result)){
				if(count($result) > 0){
					if($gebruikersnaam == $result['gebruikersnaam'] && password_verify($wachtwoord, $result['wachtwoord'])){
						session_start();
						$_SESSION['medewerker_id'] = $result['id']; //ipv id overal heeft een acc een medewerker id om toekomstige conflicten te voorkomen
						$_SESSION['gebruikersnaam'] = $result['gebruikersnaam'];

						header("location: medewerker_homepage.php");
					} else {
						echo "Oops.. Your username of password is incorrect try again";
					}
				}
			}
